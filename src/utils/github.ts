...
const fetchUserRepos = async (username: string): Promise<GitHubRepo[]> => {
  ...
  const reposResponse = await axiosInstance.get<GitHubRepo[]>(
    `${GITHUB_API_URL}/${username}/repos?per_page=100`,
  );
  reposCache = {
    username,
    timestamp: Date.now(),
...
export const fetchGitHubProfile = async (
  ...
  try {
    const response = await axiosInstance.get<GitHubProfile>(
...
export const fetchGitHubStats = async (
  ...
  try {
    ...
    const reposByActivity = [...repos].sort(
      (a, b) =>
