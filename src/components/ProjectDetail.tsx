    ...
    lines.forEach((line, idx) => {
      if (line.startsWith("```")) {
        if (!inCodeBlock) {
          ...
          elements.push(
            <div
              key={idx}
              className="relative my-6 rounded-xl overflow-hidden border border-white/10"
            >
      ...
      if (line.startsWith("# ")) {
        ...
        elements.push(
          <h2
            key={idx}
            className="text-xl sm:text-2xl font-poppins font-semibold text-foreground mt-10 mb-4 pb-2 border-b border-white/5"
          >
            {line.slice(3)}
          </h2>,
        );
        ...
        elements.push(
          <h3
            key={idx}
            className="text-lg font-semibold text-primary mt-6 mb-3"
          >
            {line.slice(4)}
          </h3>,
        );
        ...
        elements.push(
          <h4
            key={idx}
            className="text-base font-semibold text-foreground/90 mt-4 mb-2"
          >
            {line.slice(5)}
          </h4>,
        );
        ...
        elements.push(
          <li
            key={idx}
            className="text-[#b3bad9] text-sm leading-relaxed ml-4 list-disc"
          >
            {line.slice(2)}
          </li>,
        );
        ...
        elements.push(
          <li
            key={idx}
            className="text-[#b3bad9] text-sm leading-relaxed ml-4 list-decimal"
          >
            {line.replace(/^\d+\. /, "")}
          </li>,
        );
        ...
        elements.push(
          <blockquote
            key={idx}
            className="border-l-4 border-primary/40 pl-4 my-4 text-muted-foreground italic"
          >
            {line.slice(2)}
          </blockquote>,
        );
        ...
        if (cleaned) {
          elements.push(
            <p
              key={idx}
              className="text-[#b3bad9] text-sm sm:text-base leading-relaxed"
            >
              {cleaned}
            </p>,
          );
    ...
    <div className="min-h-screen text-foreground relative selection:bg-primary/30">
      ...
      <div className="container mx-auto px-4 py-8 sm:py-12 max-w-4xl">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
              {[
                ...
                },
                {
                  icon: Code2,
                  label: "Issues",
                  value: repoDetail.open_issues_count,
                  color: "text-primary",
                },
              ].map(({ icon: Icon, label, value, color }) => (
                <div
                  key={label}
                  className="glass-crystal rounded-xl p-4 flex flex-col items-center justify-center border border-white/5 hover:border-white/10 transition-colors"
                >
                  <Icon className={`w-5 h-5 mb-2 ${color}`} />
                  <span className="text-xl font-poppins font-light text-foreground">
                    {value}
                  </span>
          ...
          {repoDetail?.topics?.length || project?.tech?.length ? (
            <div className="flex flex-wrap gap-2 mb-8">
              {(repoDetail?.topics?.length
                ...
                <span
                  key={tag}
                  className="px-3 py-1 bg-primary/10 border border-primary/20 rounded-full text-xs font-mono text-primary uppercase tracking-wider"
                >
                  {tag}
                </span>
              ))}
        ...
        >
          <Link
            to="/projects"
            className="group flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors font-mono uppercase tracking-widest"
          >
