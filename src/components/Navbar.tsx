...
export default function Navbar() {
  ...
  const currentItem =
    navItems.find((item) => currentPath === item.path) || navItems[0];

  const flowIndex = routeFlow.findIndex((path) => path === currentItem.path);
  const previousPath =
    routeFlow[(flowIndex - 1 + routeFlow.length) % routeFlow.length];
  const nextPath = routeFlow[(flowIndex + 1) % routeFlow.length];
  const previousItem =
    navItems.find((item) => item.path === previousPath) || navItems[0];
  const nextItem =
    navItems.find((item) => item.path === nextPath) || navItems[0];
  ...
  return (
    <header className="lg:hidden fixed left-0 right-0 z-50 flex items-start justify-center pt-0 pointer-events-none">
      ...
      <motion.nav
        ...
        animate={{
          borderRadius:
            navState === "home-base" ? 0 : navState === "menu-open" ? 24 : 9999,
        }}
        transition={{
          type: "spring",
          stiffness: 220,
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="font-mono tracking-wider uppercase truncate">
                        {item.label}
                      </span>
                    </Link>
                  );
