...
export default function SpotifyWidget({ className = '', showDetails = true }: SpotifyWidgetProps) {
    ...
    if (isLoading) {
        return (
            <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className={`group relative bg-gradient-to-r from-[#313244] to-[#45475a] rounded-full shadow-lg border border-[#45475a] transition-all hover:scale-105 hover:shadow-xl ${className}`}
            >
    ...
    return (
        ...
        >
            {/* Main widget with muted gradient background */}
            <div className="relative bg-gradient-to-r from-[#313244] to-[#45475a] rounded-full shadow-lg border border-[#45475a] transition-all group-hover:border-[#a6adc8]/50 group-hover:shadow-xl">
                <div className="flex items-center gap-2 pl-2 pr-3 py-1.5">
                    {/* Music icon with subtle indication */}
                    <div className="relative">
                        {track.isPlaying ? (
                            <Play className="w-4 h-4 text-[#89b4fa] fill-[#89b4fa]" />
                        ) : (
                ...
                {/* Hover glow effect with theme consistency */}
                <motion.div
                    className="absolute inset-0 rounded-full bg-gradient-to-r from-[#a6adc8]/10 to-[#89b4fa]/10 blur-lg opacity-0 group-hover:opacity-100"
            ...
            <AnimatePresence>
                {showTooltip && showDetails && isValidTrack && (
                    <motion.div
                        ...
                        className="absolute left-full bottom-full mb-3 translate-x-10 ml-4 bg-gradient-to-br from-[#1e1e2e] to-[#313244] rounded-2xl shadow-2xl border border-[#45475a] w-72 p-4 z-50 backdrop-blur-xl"
                    >
                        {/* Fixed tooltip arrow */}
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-grow min-w-0">
                                ...
                                <motion.div
                                    ...
                                    className="flex items-center gap-2 text-[#89b4fa] text-xs font-medium mb-2"
                                >
                                ...
                                {track.url && (
                                    <motion.div
                                        ...
                                        className="flex items-center gap-2 text-[#89b4fa] text-xs group/cta hover:text-[#cba6f7] transition-colors cursor-pointer"
                                    >
